...
from api_gen.model import Api, BitConst, EnumEntry, Function, Group, OpaqueRef, Param, single_bit_entries
...
def validate(api: Api) -> list[str]:
    ...
    fields += [(naming.upper_camel(o.class_name), "a class") for o in classes]
    defined: dict[str, list[str]] = {}
    for name, what in fields:
        defined.setdefault(name, []).append(what)
    problems += [f"M.{name} would be both {' and '.join(whats)}" for name, whats in defined.items() if len(whats) > 1]
    for o in classes:
...
def _ok_const(api: Api, fn: Function) -> str:
    ...
    return naming.unprefixed_const_name(next(e for e in typed.entries if e.value == 0).name)
