...
**A1. One reserved-name set, enforced on every name**
...
**A2. Generated C identifiers unique across types, opaque tags and functions**
...
**A3. `size` required on `memory`; one size parameter per buffer**
...
**A5. Return enums must have a zero entry**
...
**A6. Strings that land inside quotes are quote-free**
...
**B2. Struct values are plain tables everywhere, recursively**
...
**B3. `dtor` becomes an explicit release method**
...
**B4. Remove emitter-side validation the model now owns**
...
**C1. `cdef()` is `declarations()` without constants**
...
**C2. Consistent enum trailing comma**
...
**C3. Header tests for the consumer include set and field alignment**
...
**D1. One version value**
- Touches: `model.py` `Api` (:100-121) add `version_value() -> int`; `emit_c.py` `_version_enum` (:105-108) → `= 0x{api.version_value():08x}`; `emit_lua.py` `_version_literal` (:47-50) → same expression.
- After: header `XY_API_VERSION = 0x01020304`; Lua `M.XY_API_VERSION = 0x01020304`.
- Test: `test_declarations` expected → `"XY_API_VERSION = 0x01020304"`; `test_module_surface` unchanged.
- Closes: DRY (shifts in two files).
- Docs: SPEC.md "the version in an anonymous enum" stays true.
- Note: the header's version spelling changes from the shift expression to one literal; a design choice the human confirms before the batch runs.

**D2. One function-doc list**
...
**E1. Tests for SPEC lines with none**
...
- **G1 (bit 31 / first version byte ≥ 128 exceed `int` as C enumerators).** Restricting ranges changes the human-designed DSL. Worth one SPEC sentence: "A bit index of 31 or a first version byte of 128 or more yields an enumerator outside `int`; such a header compiles as C++ or C23 only." The human may prefer the model restriction instead; that is a one-line change in `_bit_consts` and the version check.
