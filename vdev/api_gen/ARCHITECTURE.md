...
`Api` is a frozen dataclass tree: `BitConstGroup` of `BitConst`, `ConstGroup` of plain constants as `EnumEntry`, `StringConstGroup` of `StringConst`, `TypedConst` of `EnumEntry`, `OpaqueRef`, `Struct` of `Field`, `Function` of `Param`, and an optional `DriverData`. Tuples throughout, in document order. Each of the three untyped-constant kinds (bit, plain, string) is an array of groups carrying a docstring, the two numeric kinds a base type too, for the emitters that render one block per group; `Api.bit_consts`, `Api.consts` and `Api.string_consts` are every entry across a kind's groups in order, for everything that treats constants one by one: literal values, composition, uniqueness, pins. A composed entry (a list of terms, each an earlier same-table entry or a literal integer) carries both its resolved `value` and its `parts` exactly as written, since the header renders a sum symbolically from `parts` while every other emitter uses `value` directly; the loader refuses a bit-group sum whose terms share a bit. `Api.kind()` answers the one question every emitter asks, which category a type name belongs to, from the validated tree rather than by re-checking.
