...
- For each opaque ref with a `_ctor`, a class `M.<Class>`, the `_class` attribute in UpperCamel with no namespace prefix:
  ...
  - `new` and every method, the `_dtor`'s included, then asserts the type of each of its own arguments in order, one `assert` per argument, after the live-handle assert where there is one and before any FFI allocation or call: `type(x) == "number"` for `u32`, a typed-enum value, and a `memory` `out` argument's count; `type(x) == "number" or ffi.istype("uint64_t", x) or ffi.istype("int64_t", x)` for `u64` by value or `inout`, since a literal like `48ULL` may hold either C type; `type(x) == "string"` for a `memory` `in` or `inout` argument; `type(x) == "table" or ffi.istype("ns_s", x)` for a struct `s` by `in` or `inout`; `ffi.istype("ns_h", x)` for an opaque `h` passed by value — never a bare `type(x) == "cdata"`, which would accept any cdata regardless of its C type. The message is `"<Class>.<method>: <param> must be <expected>"`, e.g. `"Device.dma_to_device: psrc must be a string"`, `"Port.configure: cfg must be a table or xy_stats"`.
