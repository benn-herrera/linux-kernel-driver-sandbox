...
- For each opaque ref with a `_ctor`, a class `M.<Class>`, the `_class` attribute in UpperCamel with no namespace prefix:
  ...
  - `new` and every method, the `_dtor`'s included, then asserts the type of each of its own arguments in order, one `assert` per argument, after the live-handle assert where there is one and before any FFI allocation or call: `type(x) == "number"` for `u32`, a typed-enum value, and a `memory` `out` argument's count; `type(x) == "number" or type(x) == "cdata"` for `u64`; `type(x) == "string"` for a `memory` `in` or `inout` argument; `type(x) == "table" or type(x) == "cdata"` for a struct; `type(x) == "cdata"` for an opaque passed by value. The message is `"<Class>.<method>: <param> must be <expected>"`, e.g. `"Device.dma_to_device: psrc must be a string"`, `"Port.configure: cfg must be a table or cdata"`.
