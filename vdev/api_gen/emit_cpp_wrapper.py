...
def _marshal(api: Api, params: tuple[Param, ...], *, outrefs_are_locals: bool) -> tuple[list[str], list[str]]:
    """The C++ parameter list and the C call arguments. A sized `memory` is one span and
    its size parameter leaves the signature; with `outrefs_are_locals`, a non-memory
    outref is the caller's local, passed by address and absent from the signature."""
    sized = {p.size: p for p in params if p.type == "memory"}
    sig, call = [], []
    for p in params:
        if p.name in sized:
            call.append(f"{_ref_type(api, p.type)}({sized[p.name].name}.size())")
        elif p.type == "memory":
            sig.append(f"std::span<{'const ' if p.inref else ''}std::byte> {p.name}")
            call.append(f"{p.name}.data()")
        elif p.outref:
...
def _class(api: Api, opaque: OpaqueRef) -> str:
    ...
    create = (
        f"{_doc(ctor)}"
        f"  [[nodiscard]] static std::optional<{cls}> create({', '.join([*sig, f'{ret}* result = nullptr'])}) {{\n"
        f"{locals_}"
    ...
    lifetime = (
        ...
        f"      : {', '.join([*(f'{p.name}(other.{p.name})' for p in cached), f'{HANDLE_MEMBER}(std::exchange(other.{HANDLE_MEMBER}, nullptr))'])} {{}}\n"
        # The cached outrefs are const, so assignment cannot replace them.
    ...
    members = "".join(f"  const {_ref_type(api, p.type)} {p.name};\n" for p in cached)
    ctor_params = ", ".join([f"{handle_type} handle", *(f"const {_ref_type(api, p.type)}& {p.name}_" for p in cached)])
    inits = ", ".join([*(f"{p.name}({p.name}_)" for p in cached), f"{HANDLE_MEMBER}(handle)"])
    return (
        f"\n{_comment(opaque.docstring)}class {cls} {{\n public:\n{create}{lifetime}{''.join(body)}"
        + (f"\n{members}" if members else "")
        + f"\n private:\n  {cls}({ctor_params}) : {inits} {{}}\n\n  {handle_type} {HANDLE_MEMBER};\n}};\n"
    )
