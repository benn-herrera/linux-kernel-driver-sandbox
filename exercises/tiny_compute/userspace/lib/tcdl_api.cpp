//
// tcdl_api.cpp: Tiny Compute Device Library API implementation
//
// hand-authored file implements tcdl_api.h, which is generated by api_gen
// from api_def/tcdl_api.adef.toml - see vdev/justfile recipe 'generate'
//
#include <cerrno>
#define TCDL_IMPL
#include "tcdl_api.h"
...
tcdl_result tcdl_compute_factorial(tcdl_handle htcd, uint32_t arg, uint32_t* pfact) {
  ...
  const auto fd = h2fd(htcd);
  if (ioctl(fd, TCD_IOC_COMPUTE, IOC_PARAM(arg))) {
    return errno_to_result();
  }
  *pfact = arg;
  return TCDL_OK;
